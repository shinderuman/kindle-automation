...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "new_release_paper_detailはISBN asinとauthor必須", job: validBaseJob(KindNewReleasePaperDetail, Target{ASIN: "1234567890123", AuthorName: "海李"}), wantErr: nil},
		{name: "new_release_paper_detailのasin空はMissingField", job: validBaseJob(KindNewReleasePaperDetail, Target{AuthorName: "海李"}), wantErr: ErrMissingField},
		{name: "new_release_paper_detailのauthor空はMissingField", job: validBaseJob(KindNewReleasePaperDetail, Target{ASIN: "1234567890123"}), wantErr: ErrMissingField},
		{name: "new_release_paper_detailのasin形式不正はInvalidASIN", job: validBaseJob(KindNewReleasePaperDetail, Target{ASIN: "short", AuthorName: "海李"}), wantErr: ErrInvalidASIN},
...
func TestMessageGroup(t *testing.T) {
	tests := []struct {
		...
		{KindNewReleasePaperDetail, "amazon-requests"},
...
func TestEncodeOmitsUnusedTargetFields(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "new_release_paper_detailはtargetにasinとauthor_nameを出力する",
			job:    validBaseJob(KindNewReleasePaperDetail, Target{ASIN: "1234567890123", AuthorName: "海李"}),
			keys:   []string{"asin", "author_name"},
			extras: []string{"source_asin", "product", "gist_type"},
		},
		{
...
func TestEncodeDecodeRoundTrip_AllKinds(t *testing.T) {
	...
	cases := []struct {
		...
		{"new_release_paper_detail", validBaseJob(KindNewReleasePaperDetail, Target{ASIN: "1234567890123", AuthorName: "海李"})},
